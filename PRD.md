...
- **Autenticação (apartada, dois fatores):** qualquer e-mail com domínio exatamente `@atento.com` (recusa subdomínios e `atento.com.br`) **+ código de acesso exclusivo do painel** (5 códigos `ATN-…` com verificadores próprios no corpus `CG` de `admin-guard.js`; pipeline salt→SHA-256→transformação idêntico ao `guard.js`, cujas 5 chaves dos formulários permanecem independentes e intocadas). Entrada pela Home (card **Configurações**, no lugar do card Termos de Aceite) e pela sidebar. Sessão em `sessionStorage` (60 min, renovada por atividade). Proteção client-side — mesma natureza e limitações do `guard.js`.
