...
| `native-docs.js` | **Gerador nativo de PDFs** (engine): definição declarativa do documento, validação, sistema de coordenadas (X da esquerda, Y do topo, pt), renderer pdf-lib, versionamento/hash, comparador (visual e **geométrico**), extração de mobiliário (textos/réguas/caixas/tarjas/imagens/caixas de marcação) e importação de referência. Usado pelo painel (seção **Documentos Nativos**) e pronto para a aplicação pública quando um documento estiver em modo `native`. Manual: `Docs/documentos-nativos.md`. |
| `ficha_cadastral_nativo.json` | **Documento nativo do F-075** (linha de base do repositório): 165 elementos — 102 blocos de texto, 17 caixas de marcação, 17 réguas/caixas, 9 imagens e os 37 elementos do schema. Regerado por `scripts/gerar-nativo-f075.mjs`. O painel o carrega quando o overlay não tem o documento e o exporta em *Dados & Backups*. |
| `f075_nativo_01.png` … `f075_nativo_09.png` | Imagens do mobiliário do F-075 (logo, ONE, carimbo e faixas de tabela) recortadas da referência. Ficam na raiz porque o painel só aceita assets de primeiro nível. |
| `scripts/referencia/f075-pagina1.json` | **Referência congelada** do F-075 p.1: texto (baseline/tamanho/família/cor), réguas, caixas e imagens, com o que foi ignorado (recortes, máscaras). Gerada pelo mesmo extrator do painel; é o gabarito do comparador geométrico. |
| `scripts/gerar-nativo-f075.mjs` | Monta a definição a partir do schema + referência congelada + tipografia oficial e **falha (exit 1) se o deslocamento sair da tolerância**. `--conferir` só mede, sem gravar. |
| `scripts/referencia/f075-tipografia.json` | **Tipografia oficial por run** do F-075, extraída do documento editável (Arial 72 blocos, Arial Narrow 9, Wingdings 17 caixas). Versionada; regerada por `scripts/extrair-tipografia-f075.mjs "<caminho do .docx>"` — o `.docx` em si é da empresa e **não** entra no repositório. |
| `scripts/extrair-tipografia-f075.mjs` | Lê o `.docx` oficial (ZIP lido sem dependência, só `zlib`) e grava o extrato de tipografia. É o que permite ao gerador declarar a fonte run a run em vez de aceitar a substituição silenciosa que a conversão do PDF fez. |
...
> **Gerador nativo (`native-docs.js`)** usa o sistema oposto: **Y medido do topo** (a conversão para o espaço do pdf-lib acontece em uma única função, `NativeDocs.converterY`). O bootstrap do schema converte `y_pdf → y_topo` automaticamente, então os arquivos `*_campos.json` **continuam** no padrão acima e nada precisa ser re-medido. O perfil de texto da aplicação (fonte 9 pt, `x + 0,5`, `min(altura × 0,78; fonte × 1,12)`, ajuste de 9 pt acima de `y = 120`) está em `NativeDocs.PERFIL_APP` e é conferido **contra o `ficha_cadastral.html`** pela auditoria: se o código de geração mudar, a auditoria acusa.
...
- `run-test.mjs` — suíte de testes do projeto (Testes 1–18; o 14 cobre o painel v2, o 15/16 o painel v3/Field Builder, o 17 a auditoria de dados e o 18 o gerador nativo de PDFs — fidelidade do bootstrap, renderer, validação, versões, comparador e importação de referência); executar com `node scripts/run-test.mjs`.
...
- Documentos Nativos (gerador de PDF): seção **▣ Documentos Nativos** — engine em `native-docs.js` (definição `{ schemaVersion, documentId, documentVersion, page, metadados, assets, elementos }`, elementos `text/field/line/rectangle/circle/ellipse/image/table/checkbox/signature/group`, coordenadas em pt com Y do topo). Subseções: Templates Nativos (criar do schema, editor com preview do PDF **real**, snap/zoom, propriedades numéricas, camadas, agrupar, exclusão), Comparação (template oficial × nativo, com sobreposição, % de diferença e deslocamento estimado), Importar PDF (referência: extrai página, **textos fixos, réguas, caixas, tarjas, caixas de marcação e imagens** da geometria real dos operadores de desenho; imagens saem como PNG para versionar na raiz e ficam em memória para o preview; tudo entra como REQUER CALIBRAÇÃO até ser confirmado — o que a extração ignora é declarado no relatório: recortes, máscaras transparentes, curvas aproximadas), Versões (congelamento com hash SHA-256 e restauração append-only), Logs (§41) e Assets & Fontes (§14/§15, com a limitação de fontes documentada). Overlay: `docs_nativos` (`{ meta, definicao, versoes, log }` por documento) — chave nova e opcional, com pendência própria e diff de importação. Modo híbrido por documento: `external` (padrão, template atual) × `native` (só efetivo com status PUBLICADO e definição válida — §49, sem regressão). Configurações: `documentos.modo_padrao`, `documentos.tolerancia_visual_pt`, `documentos.exigir_confirmacao_importados`. O bootstrap converte o schema real preservando a geometria (mesma baseline e mesmo `x`), o que a auditoria confere contra o `ficha_cadastral.html`. Manual: `Docs/documentos-nativos.md`; prova: Teste 18 + auditoria 13.*.
...
*Última atualização: gerador nativo de PDFs (`native-docs.js`) integrado ao painel na seção Documentos Nativos; assistência com template único `DECLARACAO PLANO DE SAUDE.pdf` e municípios via API kstr.*
