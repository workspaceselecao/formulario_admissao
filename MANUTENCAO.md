...
- Publicação (§23/§24): Dashboard lista **Alterações Pendentes**; publicar exige validação (erro crítico trava; aviso confirma) e registra marco append-only (`pendentesMarcados`); descartar remove patches pendentes preservando cidades novas sem substituto.
