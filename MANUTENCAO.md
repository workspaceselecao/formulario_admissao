...
| `ficha-cadastral-embutido/` | **Prova de conceito e template do F-075**: `extract_template.py` (extrai `template.json` + `assets/*.png` de qualquer PDF oficial — generaliza para os 4 documentos), `template.json` (página, 9 imagens, barras, caixas brancas, 262 textos estáticos, 17 marcações) e `generate.js` (gerador standalone Node). **`template.json` e `assets/*.png` são fonte versionada** (o painel os busca via HTTP no deploy estático); só `output.pdf` é artefato, ignorado pelo `.gitignore`. |
