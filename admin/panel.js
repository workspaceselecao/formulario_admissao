...
(function (global) {
  ...
  const state = {
    ...
    dnZoom: 100,                 // zoom visual (não altera a escala real)
    docs_logs: {},               // log de alterações por documentId (sessão)
  };
  ...
  // ══════════════════════════════════════════════════════
  async function coletarProblemas() {
    ...
    // Gerador embarcado: valida o template de cada documento embarcado do repositório.
    if (global.EmbeddedDocs) {
      for (const fonte of (typeof DN_FONTES !== "undefined" ? DN_FONTES : [])) {
        try {
          const rT = await fetch(urlRepositorio(fonte.templateDir + "/template.json"), { cache: "no-store" });
          if (!rT.ok) { avisos.push("Documento embarcado " + fonte.nome + ": template.json inacessível (" + rT.status + ")."); continue; }
          const template = await rT.json();
          const schemaCampos = (state.docData[fonte.docKey] || {}).json && state.docData[fonte.docKey].json.campos;
          const v = global.EmbeddedDocs.validarTemplate(template, schemaCampos);
          const prefixo = "Documento embarcado " + fonte.nome + ": ";
          for (const e of v.erros) criticos.push(prefixo + e);
          for (const a of v.avisos) avisos.push(prefixo + a);
        } catch (e) {
          avisos.push("Documento embarcado " + fonte.nome + ": falha ao validar o template (" + e.message + ").");
        }
      }
    }
  ...
  async function validarAntesDePublicar() {
    const { criticos, avisos } = await coletarProblemas();
    if (criticos.length) {
  ...
  const CONFIG_DEFS = [
    ...
    { key: "documentos.tolerancia_visual_pt", label: "Tolerância da comparação (pt)", categoria: "Documentos", tipo: "int", min: 0, max: 20, default: 1, efeito: "Comparador template oficial × PDF embarcado (pixels e deslocamento estimado)" }
  ];
  /**
  ...
  /** Exporta o template do documento embarcado (versionar no repositório). */
  function exportarTemplateEmbarcado(docId) {
    const N = dnNativo();
  ...
  async function importarConfig(file) {
    try {
      ...
      function blocoMap(key) {
        ...
        const resumo = function (v) { return resumoValor(v); };
  ...
  function construirIndiceBusca() {
    ...
    // Documentos embarcados: cada documento do inventário
    for (const f of (typeof DN_FONTES !== "undefined" ? DN_FONTES : [])) {
      itens.push({ tipo: "Documento embarcado", titulo: f.nome, sub: "template " + f.templateDir + " · geração embarcada", acao: function () { showSection("documentos"); dnSelecionarDoc(f.documentId); } });
    }
    return itens;
  }
  ...
  // ══════════════════════════════════════════════════════
  // GERADOR EMBARCADO DE PDFs (ficha-cadastral-embutido)
  //
  // O PDF é gerado pela própria aplicação a partir do TEMPLATE DECLARATIVO
  // (template.json + assets PNG extraídos do documento oficial) + os campos
  // do schema de coordenadas — NENHUM PDF externo é carregado nem enviado.
  // O engine (validação estrutural, renderer pdf-lib, perfil de texto da
  // aplicação) vive em /embedded-docs.js — sem DOM, reutilizável pela
  // aplicação pública. Aqui fica a camada administrativa: preview do PDF
  // REAL gerado, localizador de campos (com atalho para o Editor Visual,
  // que edita as MESMAS chaves de overlay), comparação e log.
  //
  // Regras respeitadas: nada de localStorage (§2), os JSONs do repositório
  // continuam intocados e o template/POC é versionado no repositório —
  // sem overlay de definição, sem versões no painel, sem calibração manual.
  // ══════════════════════════════════════════════════════

  /** Documentos embarcados: template.json + assets por documento. */
  const DN_FONTES = [
    {
      documentId: "f075",
      nome: "Ficha Cadastral (F-075)",
      docKey: "ficha_cadastral",
      schemaArquivo: "ficha_cadastral_campos.json",
      templateDir: "ficha-cadastral-embutido",
      pdfFile: FICHA_PDF
    }
  ];
  ...
  let dnTemplateCache = {};   // documentId → { template, imagens, quando }
  let dnFolhasSel = null;     // folha selecionada no localizador

  function dnNativo() { return global.EmbeddedDocs || null; }
  function dnFontePor(docId) { return DN_FONTES.filter(function (f) { return f.documentId === docId; })[0] || null; }
  ...
  async function dnCarregarTemplate(fonte) {
    ...
    const cache = dnTemplateCache[fonte.documentId];
    if (cache) return cache;
    const rT = await fetch(dnUrlTemplate(fonte, "template.json"), { cache: "no-store" });
    if (!rT.ok) throw new Error("template.json não encontrado (" + rT.status + ") em " + fonte.templateDir + "/");
    const template = await rT.json();
    const imagens = {};
    for (const img of (template.images || [])) {
      if (!img || !img.file) continue;
      try {
        const r = await fetch(dnUrlTemplate(fonte, img.file), { cache: "no-store" });
  ...
  /** Invalida o cache do template (usado quando o arquivo muda no repositório). */
  function dnInvalidarTemplate(docId) { if (docId) delete dnTemplateCache[docId]; else dnTemplateCache = {}; }

  /**
   * Dados fictícios para o preview/PDF de teste. Só preenche folhas com
   * correspondência conhecida (nada é inventado): o resto fica em branco,
   * como ficaria para um candidato que não informou aquele dado.
   */
  const DN_MAPA_TESTE = {
  ...
  function dnDadosDeTeste(schemaCampos) {
    const N = dnNativo();
    const dados = {};
    if (!N || !schemaCampos) return dados;
    for (const f of N.folhasComCoordenadas(schemaCampos)) {
      const chave = String(f.path).split(".").pop().toLowerCase();
      const alvo = DN_MAPA_TESTE[chave];
      if (alvo && DADOS_TESTE[alvo] != null) dados[f.path] = DADOS_TESTE[alvo];
    }
    return dados;
  }

  /** Validação do template + contagem de campos (usada no resumo e na validação). */
  function dnValidacao(fonte, schemaCampos, template) {
    const N = dnNativo();
    if (!N) return { ok: false, erros: ["Engine /embedded-docs.js não carregou."], avisos: [], resumo: null };
    return N.validarTemplate(template, schemaCampos);
  }

  /** Preview = o PDF de verdade, gerado agora pela engine e renderizado com pdf.js. */
  async function dnRenderPreview() {
    const N = dnNativo();
    const fonte = dnFonteSel();
    const canvas = $("dnCanvas");
    if (!N || !fonte || !canvas) return;
    if (!global.PDFLib || !global.pdfjsLib) {
      toast("pdf-lib/pdf.js não carregaram (CDN) — o preview fica indisponível.", false);
      return;
    }
  ...
  /** Cabeçalho: validação do template, camadas e resultado da última geração. */
  function dnRenderResumo(gerado, pacote) {
    const N = dnNativo();
    const box = $("dnResumo");
    const fonte = dnFonteSel();
    if (!box || !N || !fonte) return;
    const schemaCampos = (state.docData[fonte.docKey] || {}).json && state.docData[fonte.docKey].json.campos;
    const v = dnValidacao(fonte, schemaCampos, pacote && pacote.template);
    const r = v.resumo;
    box.innerHTML = (r
      ? '<div class="legend-selos">' +
        '<span class="badge ok">template ' + esc(fonte.templateDir) + "/template.json</span> " +
        '<span class="badge muted">' + dnArred(r.pagina.width) + " × " + dnArred(r.pagina.height) + " pt · " + r.paginas + " página(s)</span> " +
        '<span class="badge muted">' + r.campos + " campo(s) no schema</span>" +
        '<span class="badge muted">' + r.camadas.texts + " texto(s) fixo(s) · " + r.camadas.images + " imagem(ns) · " + r.camadas.checkboxes + " marcação(ões)</span>" +
        (v.avisos.length ? ' <span class="badge warn">' + v.avisos.length + " aviso(s)</span>" : "") +
        "</div>"
      : "") +
      (v.erros.length ? '<div class="notice err" style="margin-top:6px">' + esc(v.erros.slice(0, 3).join(" · ")) + "</div>" : "") +
      (v.avisos.length && !v.erros.length ? '<div class="notice warn" style="margin-top:6px">' + esc(v.avisos.slice(0, 3).join(" · ")) + "</div>" : "") +
  ...
  function dnRenderPaginas() {
    ...
    const sel = $("dnPage");
    const fonte = dnFonteSel();
    if (!sel || !N || !fonte) return;
    let nPaginas = 1;
    const pacote = dnTemplateCache[fonte.documentId];
    if (pacote && Array.isArray(pacote.template.paginas) && pacote.template.paginas.length) nPaginas = pacote.template.paginas.length;
    const atual = Math.min(dnPaginaAtual(), nPaginas);
    sel.innerHTML = Array.from({ length: nPaginas }, function (_, i) {
      return '<option value="' + (i + 1) + '"' + ((i + 1) === atual ? " selected" : "") + ">Página " + (i + 1) + " de " + nPaginas + "</option>";
    }).join("");
  }

  /** Localizador de campos: folhas do schema (mesma chave do Editor Visual/overlay). */
  function dnRenderLista() {
    const N = dnNativo();
    const fonte = dnFonteSel();
    const box = $("dnLista");
    const cont = $("dnListaContagem");
    if (!box || !N || !fonte) return;
    const schemaCampos = (state.docData[fonte.docKey] || {}).json && state.docData[fonte.docKey].json.campos;
    if (!schemaCampos) { box.innerHTML = '<div class="section-desc">Schema não carregado.</div>'; return; }
    const folhas = N.folhasComCoordenadas(schemaCampos);
    const busca = ((($( "dnBuscaElemento") || {}).value) || "").toLowerCase();
    const norm = function (s) { return String(s || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, ""); };
    const t = norm(busca).trim();
    const filtradas = t ? folhas.filter(function (f) {
      return norm(f.path).indexOf(t) !== -1 || norm(f.label).indexOf(t) !== -1 ||
        String(dnArred(f.coordenadas.x)).indexOf(t) !== -1 || String(dnArred(f.coordenadas.y)).indexOf(t) !== -1;
    }) : folhas;
    if (cont) cont.textContent = (t ? filtradas.length + " de " : "") + folhas.length + " campos" + (t ? ' (filtro "' + busca + '")' : "");
    const overlayMap = state.overlay[DOCS[fonte.docKey].overlayKey] || {};
    box.innerHTML = filtradas.slice(0, 300).map(function (f) {
      const pend = overlayMap[f.path] ? Object.keys(overlayMap[f.path]).length : 0;
      const marcado = dnFolhasSel && dnFolhasSel.path === f.path;
      return '<button type="button" role="option" class="dn-item' + (marcado ? " sel" : "") + '" data-dn-folha="' + esc(f.path) + '">' +
        '<span class="dn-tipo">p' + esc(f.pagina) + "</span>" +
        "<span class=\"dn-id\">" + esc(f.label || f.path) + "</span>" +
        '<span class="dn-det">' + esc(f.path) + " · x " + dnArred(f.coordenadas.x) + ", y " + dnArred(f.coordenadas.y) + (pend ? ' · <strong style="color:var(--warn)">' + pend + " ajuste(s) no overlay</strong>" : "") + "</span></button>";
    }).join("") || '<div class="section-desc">Nenhum campo' + (t ? " para esta busca" : "") + ".</div>";
    box.querySelectorAll("[data-dn-folha]").forEach(function (b) {
      b.addEventListener("click", function () {
        dnFolhasSel = filtradas.filter(function (f) { return f.path === b.getAttribute("data-dn-folha"); })[0] || null;
        dnRenderLista();
        dnRenderProps();
      });
    });
  ...
  /** Painel do campo selecionado: coordenadas (somente leitura) + atalho para o editor. */
  function dnRenderProps() {
    const f = dnFolhasSel;
    const ids = ["dnX", "dnY", "dnW", "dnH", "dnPagina", "dnRotacao", "dnIrEditor"];
    for (const id of ids) { const e = $(id); if (e) e.disabled = !f; }
    const info = $("dnElInfo");
    const titulo = $("dnElTitulo");
    if (titulo) titulo.textContent = f ? "Campo selecionado" : "Campo selecionado";
    if (info) info.innerHTML = f
      ? "<strong>" + esc(f.label || f.path) + "</strong> · chave do overlay: <code>" + esc(f.path) + "</code>" +
        (f.tipo ? " · tipo: " + esc(f.tipo) : "")
      : "Selecione um campo na lista. O botão abaixo abre este campo no Editor Visual — que edita a MESMA coordenada usada pela geração embarcada.";
    if (!f) return;
    const c = f.coordenadas || {};
    const set = function (id, v) { const e = $(id); if (e) e.value = v; };
    set("dnX", dnArred(c.x));
    set("dnY", dnArred(c.y));
    set("dnW", dnArred(c.largura != null ? c.largura : c.width));
    set("dnH", dnArred(c.altura != null ? c.altura : c.height));
    set("dnPagina", f.pagina);
    const overlayMap = state.overlay[DOCS[dnFonteSel().docKey].overlayKey] || {};
    const pend = overlayMap[f.path] ? Object.keys(overlayMap[f.path]).length : 0;
    set("dnRotacao", pend ? pend + " ajuste(s) não publicado(s)" : "sem pendência");
  }

  /** Abre o campo selecionado no Editor Visual (mesma chave de overlay). */
  function dnIrEditor() {
    const f = dnFolhasSel;
    const fonte = dnFonteSel();
    if (!f || !fonte) return;
    showSection("coordenadas");
    const sel = $("edDoc");
    if (sel && sel.value !== fonte.docKey) {
      sel.value = fonte.docKey;
      loadDocForEditor(fonte.docKey).catch(function (e) { toast("Falha ao carregar template: " + e.message, false); });
    } else if (typeof renderEditorPage === "function") {
      renderEditorPage(fonte.docKey);
    }
    const st = state.docData[fonte.docKey];
    if (st && st.flat) {
      const alvo = st.flat.filter(function (x) { return x.key === f.path; })[0];
      if (alvo) {
        st.page = alvo.pagina || 1;
        st.sel = alvo;
        renderEditorPage(fonte.docKey);
        toast('Campo "' + (f.label || f.path) + '" selecionado no Editor Visual.');
      } else {
        toast("Campo localizado — use a busca do Editor Visual.", false);
      }
    }
  }

  /** §32 — PDF de teste de verdade (independente do template oficial). */
  async function dnGerarPdfTeste() {
    ...
    try {
      ...
      toast("Falha ao gerar o PDF embarcado: " + e.message, false);
    }
  }
  ...
  // ── Comparação (regressão visual: original oficial × PDF embarcado) ──
  ...
  /**
   * Teste de regressão visual: divergência + deslocamento estimado.
   * A divergência só olha as áreas impressas (fundo branco não conta) e o
   * deslocamento é a translação que melhor alinha o desenho — duas páginas
   * idênticas devolvem 0,0, não ruído.
   */
  ...
  async function dnComparar() {
    ...
    if (!N || !fonte || !global.pdfjsLib) { toast("Comparação indisponível (pdf.js/template).", false); return; }
    ...
    try {
      ...
      await dnRenderPdfEm(pdfEmb, pagina, $("dnCmpNativo"), escala);
      const r = dnMedirDiferenca($("dnCmpOriginal"), $("dnCmpNativo"), escala, tol);
      dnCmpBlobs = { original: fonte.pdfFile, nativo: funcNativoBlob(gerado.bytes) };
      $("btnDnCompararAbrir").disabled = false;
      const okDesloc = r && Math.abs(r.dx) <= tol && Math.abs(r.dy) <= tol;
      const semTexto = r && r.amostras === 0;
      box.innerHTML = '<div class="notice ' + (semTexto ? "warn" : (r && r.percentual < 5 && okDesloc ? "ok" : "err")) + '">' +
        "<strong>Diferença nas áreas impressas: " + (r ? r.percentual : "—") + "%</strong> · " +
        "deslocamento estimado do embarcado: <strong>dx " + (r ? r.dx : "—") + " pt · dy " + (r ? r.dy : "—") + " pt</strong> " +
  ...
  /** Log do documento: trilha administrativa da sessão (append-only). */
  function dnRenderLogs() {
    const box = $("dnLogs");
    const fonte = dnFonteSel();
    if (!box) return;
    const log = (fonte && state.docs_logs[fonte.documentId]) || [];
    box.innerHTML = log.length
      ? log.map(function (l) {
        return '<div class="diff-line"><span class="path">' + esc(fonte.documentId) + "</span> · " +
          esc(l.autor) + " · " + esc(String(l.quando || "").replace("T", " ").slice(0, 16)) + " — " + esc(l.alteracao) + "</div>";
      }).join("")
      : '<div class="section-desc">Sem alterações registradas nesta sessão para este documento. O template embarcado é versionado no repositório (git) — a trilha de commits é o histórico definitivo.</div>';
  }

  function dnRegistrarLog(fonte, alteracao) {
    if (!fonte || !alteracao) return;
    if (!state.docs_logs[fonte.documentId]) state.docs_logs[fonte.documentId] = [];
    state.docs_logs[fonte.documentId].unshift({ quando: new Date().toISOString(), autor: dnAutor(), alteracao: alteracao });
  }
  ...
  function renderDocumentos() {
    ...
    if (!N) {
      boxResumo.innerHTML = '<div class="notice err">O engine <code>/embedded-docs.js</code> não carregou — a geração embarcada fica indisponível (verifique a CDN/arquivo).</div>';
      return;
    }
    const ids = DN_FONTES.map(function (f) { return f.documentId; });
    sel.innerHTML = ids.map(function (id) {
      const f = dnFontePor(id);
      return '<option value="' + esc(id) + '"' + (sel.value === id ? " selected" : "") + ">" + esc(f ? f.nome : id) + "</option>";
    }).join("");
  ...
  /** Ligações de eventos da seção Documentos Embarcados (uma vez, no init). */
  function dnInstalarHandlers() {
    ...
    const pag = $("dnPage");
    if (pag) pag.addEventListener("change", function () { dnRenderPreview().catch(function () { }); });
    const zoom = $("dnZoom");
    ...
    if (busca) busca.addEventListener("input", function () { dnRenderLista(); });

    const ligar = function (id, fn) { const b = $(id); if (b) b.addEventListener("click", fn); };
    ligar("dnIrEditor", dnIrEditor);
  ...
  function renderSistema() {
    ...
    $("sisSobre").innerHTML = "Painel Administrativo — " + esc(configGet("geral.nome_sistema")) + ".<br>" +
      ...
      "Geração embarcada: " + (typeof DN_FONTES !== "undefined" ? DN_FONTES.length : 0) + " documento(s) · engine " + (dnNativo() ? "carregado" : "ausente (embedded-docs.js)") + ".<br>" +
  ...
  async function init() {
    ...
    const btnTemplateEmb = $("btnExpTemplateEmbarcado");
    if (btnTemplateEmb) btnTemplateEmb.addEventListener("click", function () { exportarTemplateEmbarcado(dnDocIdSel() || "f075"); });
  ...
  global.AdminPanel = {
    ...
    __teste: {
      ...
      // Documentos embarcados: inventário + funções puras para a suíte e a auditoria
      DN_FONTES: DN_FONTES,
      dnDadosDeTeste: dnDadosDeTeste,
      dnCarregarTemplate: dnCarregarTemplate,
