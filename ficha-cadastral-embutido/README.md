...
- `output.pdf` — exemplo gerado **apenas na execução direta** (`node generate.js`) e **ignorado pelo git** (`.gitignore`): é artefato, não fonte. `template.json` e `assets/*.png`, ao contrário, **são fonte e ficam versionados** — o painel (`admin/`) os consome via HTTP no deploy estático (Vercel, sem build), a auditoria (`scripts/audit-panel.mjs` § 13.6) confere que existem no repositório e que não caíram no `.gitignore`. Gerar on-the-fly no CI foi descartado: exigiria Python + `pdfplumber` + o PDF oficial no ambiente de build, e o valor do template está justamente em ser revisável (diff) e auditável no repositório.
