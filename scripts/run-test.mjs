...
// Lidos de keys-testes.txt (gitignored, seguem a convenção de keys.txt) — nada de
// chave literal é versionado; sem o arquivo, o Teste 11 é marcado como skipped.
const KEYS_FILE = join(ROOT, "keys-testes.txt");
function lerChavesTestes() {
  try { return readFileSync(KEYS_FILE, "utf8").split("\n").map(l => l.trim()).filter(l => l && !l.startsWith("#")); } catch { return []; }
}
const PANEL_CODES = lerChavesTestes().slice(5, 10);
...
async function runTests() {
  ...
  console.log("\n📋 Teste 11: Códigos exclusivos do painel");
  {
    ...
    if (!PANEL_CODES.length) {
      console.log("      ⏭️  skipped — keys-testes.txt ausente (códigos do painel não são versionados)");
    } else {
      ...
      const chavesForm = lerChavesTestes().slice(0, 5).map(k => k.replace(/[\s-]/g, ""));
      assert("nenhuma chave de formulário autoriza o painel (apartamento efetivo)",
        !pairs.some(p => chavesForm.some(k => deriveVerifierNode(k, p.s) === p.v)), "form key matches panel corpus!");
    }
...
async function testarPainelV2(assert) {
  ...
  st.overlay.configuracoes = {};
  const gateLigado = await T.coletarProblemas();
  ...
  st.overlay.configuracoes = { formularios: { exigir_template: false } };
  const gateDesligado = await T.coletarProblemas();
